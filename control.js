...
const configuration = new Configuration({
	apiKey: `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`,
});
...
`            
            ...
            case 'menu': case 'dashboard': case 'help': case 'm': {
                ...
                await sleep(500)
                bob.sendMessage(m.chat, {text: tekos})
                }
                break
            case 'public': {
...
_Hasil :_ `
                 ...
                     }
                    ...
                    if (isQuotedImage || isImage) {
                    var rand = otpkode(5)
                    var down = await downloadAndSaveMediaMessage("image", rand + `.jpg`)
                    jadianime(fs.readFileSync(rand + `.jpg`)).then ( data => {
                    bob.sendMessage(m.chat, {image: {url: `https://www.drawever.com` + data.urls[1]}, caption: `Sukses Convert To Anime`})
                    fs.unlinkSync(rand + `.jpg`)
                    })
                    }
                    } catch (error) {
                        reply(`Maaf. Wajah tidak Terdeksi.`)
                    }
                    }
                    break*/
                    case 'toimg':{
...
fakereply(rules)
                    ...
                    try {
                        ytmp4(q).then ( data => {
                                var teksyutup = `*[ DOWNLOAD YOUTUBE MP4 ]*\n\n 📛 Judul : ${data.title}\n🔗 Link : ${q}\n\nSedang Mengirim...`
                                bob.sendMessage(m.chat, {image: {url: data.thumbnail}, caption: teksyutup})
                                bob.sendMessage(m.chat, {video: {url: data.urls.hd}})
                            })
                        } catch (e) {
                            ytmp4(q).then ( data => {
                                var teksyutup = `*[ DOWNLOAD YOUTUBE MP4 ]*\n\n 📛 Judul : ${data.title}\n🔗 Link : ${q}\n\nSedang Mengirim...`
                                bob.sendMessage(m.chat, {image: {url: data.thumbnail}, caption: teksyutup})
                                bob.sendMessage(m.chat, {video: {url: data.urls.sd}})
                            })
                        }
                    }
                    break
                   ...
                   /* case 'ytmp3':{
                    ...
                    limitAdd(sender, limit)
                        ...
                        Download(q).then ( data => {
                            bob.sendMessage(m.chat, {document: {url:data.mp3[0].url}, mimetype: 'audio/mp4', fileName: data.title + `.mp3`, caption: monospace(data.title)})
                        })
                    } catch (e) {
                        reply(`Hayolo bot rusak`)
                    }
                    }
                    break*/
                    case 'tt': case 'tiktok':  {
                        ...
                        if (m.text) {
                            ...
                            try {
                                const response = await openai.createChatCompletion({
                                    model: "gpt-3.5-turbo",
                                    messages: [
                                        {role: "system", content: `Ini Adalah Jojo Bot WhatsApp, Yang Di Ciptakan Oleh Arasya. Jam sekarang : ${jam}, 
                                        tanggal sekarang : ${tgl}, nama user : ${pushname}, nomer user : ${sender.split("@")[0]}, jika nomer user adalah 6288214772441 maka itu adalah pemilik bot ini yaitu Arasya`},
